...
function _Catalyst:Window(opt)
    ...
    local function applyAllVisuals()
        ...
        if _Catalyst._customAccent and _Catalyst.Flags["_accent"] then

            setAccent(_Catalyst.Flags["_accent"])
    ...
    function Window:LoadConfig(name)
        ...
        for flag, val in pairs(data) do
            ...
            if flag ~= "_theme" and flag ~= "_accent" and flag ~= "_wmpos" and flag ~= "_wmscale" then
        ...
        if data["_wmpos"] ~= nil and type(data["_wmpos"]) == "table" then

            local p = data["_wmpos"]

            if p.sx ~= nil then

                wmFrame.Position = UDim2.new(p.sx, p.ox, p.sy, p.oy)

                _Catalyst.Flags["_wmpos"] = p

            end

        end

        if data["_wmscale"] ~= nil then

            applyWmScale(data["_wmscale"])

            _Catalyst.Flags["_wmscale"] = data["_wmscale"]
    ...
    sApi:Slider("Watermark Scale", "Resize the watermark", 75, 150, 100, function(v)
...
Make it so the watermark text doesnt go off into ... make it fixed size. Menu icon / text on execution and re exeuction gets messed up with auto-load recent, the toggles dont change color unless u toggle on and off please fix i want it to be accent color at all times and text icon and color shit automatically fixed and working when loading and swapping between themes. watermark position /size should be saved automatically when saving config and doesnt need its own button, only need buttons for reset. no -- notes send snippets to replace
